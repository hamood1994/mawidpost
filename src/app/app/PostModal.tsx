...
interface Comment { id: string; author_email: string | null; body: string; created_at: string }
...
export default function PostModal({ ctx, post, onClose, onChanged }: { ctx: Ctx; post: Post; onClose: () => void; onChanged: () => void }) {
  ...
  const loadComments = () =>
    supabase.from("post_comments").select("id, author_email, body, created_at").eq("post_id", post.id).order("created_at").then((r) => setComments((r.data as Comment[]) ?? []));
  ...
  async function addComment(body: string) {
    if (!body.trim()) return;
    await supabase.from("post_comments").insert({ post_id: post.id, org_id: ctx.org.id, author_email: ctx.email, body: body.trim() });
    setNote("");
    loadComments();
  }
  ...
  return (
    <div className="overlay" onClick={onClose}>
      <div className="modal wide" onClick={(e) => e.stopPropagation()}>
        ...
        {post.status === "pending" && <div className="msg warn">بانتظار موافقة المالك أو المدير.</div>}
        ...
        <div className="field">
          <label>ملاحظات الفريق</label>
          {comments.map((c) => (
            <div key={c.id} style={{ fontSize: 13, marginBottom: 6 }}>
              <span style={{ color: "var(--muted)" }}>{c.author_email ?? "—"}: </span>{c.body}
            </div>
          ))}
          <div className="row">
        ...
        <div className="actions" style={{ flexWrap: "wrap" }}>
          ...
          {post.status === "pending" && ctx.isAdmin && (
            <>
              <button className="btn" disabled={busy} onClick={async () => { if (note.trim()) await addComment(note); await update({ status: "draft" }); }}>رفض (مسودة)</button>
